...
public class WrapMongoIterable extends BaseWrapper<MongoIterable<Document>> implements Traversable {
    ...
    @Signature
    public void forEach(Invoker callback) {
        try (MongoCursor<Document> cursor = getWrappedObject().iterator()) {
            while (cursor.hasNext()) {
                callback.callNoThrow(Memory.wrap(getEnvironment(), cursor.next()));
            }
        }
    }
